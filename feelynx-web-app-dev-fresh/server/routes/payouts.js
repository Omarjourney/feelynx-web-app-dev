...
import { payoutSchemas, withValidation } from '../utils/validation';
const stripeSecret = process.env.STRIPE_SECRET_KEY;
const stripe = stripeSecret ? new Stripe(stripeSecret) : null;
export const router = Router();
router.post('/onboard', withValidation(payoutSchemas.onboard), async (req, res) => {
  if (!stripe) {
...
router.post('/request', withValidation(payoutSchemas.request), async (req, res) => {
  if (!stripe) {
    return res.status(500).json({ error: 'Stripe not configured' });
  }
