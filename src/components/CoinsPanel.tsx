...
export const CoinsPanel = () => {
  ...
  const recentTransactions = [
    ...
    { id: 2, type: "spent", amount: -150, description: "Video Call with ArianaVex", date: "1 day ago" },
    ...
    <div className="container mx-auto p-4 space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {packages.map((pkg) => (
            <Card key={pkg.id} className={`relative bg-gradient-card ${pkg.popular ? 'ring-2 ring-primary shadow-premium' : ''}`}>
              {pkg.popular && (
            ...
            {recentTransactions.map((transaction) => (
              <div key={transaction.id} className="flex items-center justify-between p-3 bg-secondary rounded-lg">
                <div className="flex items-center space-x-3">
                  <div className={`text-2xl ${transaction.type === 'purchase' ? 'text-live' : 'text-destructive'}`}>
                ...
                <div className={`font-bold ${transaction.amount > 0 ? 'text-live' : 'text-destructive'}`}>
