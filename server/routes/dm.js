...
import { dmSchemas, withValidation } from '../utils/validation';
const router = Router();
router.post(
  '/threads',
  authenticateToken,
...
router.post(
  ...
  async (req, res) => {
    const { id } = req.params;
    const { cipher_text, nonce, recipientId, burnAfterReading } = req.body;
    const { data, error } = await supabase
      .from('dm_messages')
      .insert({
        thread_id: id,
        ...
        nonce,
        burn_after_reading:
          burnAfterReading !== null && burnAfterReading !== void 0 ? burnAfterReading : false,
      })
      .select()
      .single();
    if (error) {
