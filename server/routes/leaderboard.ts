...
const writeLeaderboard = async (entries: LeaderboardEntry[]) => {
  await fs.mkdir(path.dirname(DATA_FILE), { recursive: true });
  await fs.writeFile(DATA_FILE, JSON.stringify(entries, null, 2));
};

const resolveTier = (tokens: number): LeaderboardEntry['tier'] => {
  if (tokens >= 5000) return 'Elite';
  if (tokens >= 2000) return 'Pro';
  return 'Growth';
};

const resolveStreakBadge = (streakDays: number) => {
  if (streakDays >= 14) return '🔥 Elite Streak';
  if (streakDays >= 7) return '⚡ Pro Streak';
  if (streakDays >= 3) return '✨ Growth Streak';
  return null;
};
...
router.get('/', async (_req, res) => {
  try {
    ...
    const withBadges = sorted.map((entry, index) => ({
      ...entry,
      rank: index + 1,
      streakBadge: resolveStreakBadge(entry.streakDays),
    }));
...
router.post(
  '/',
  withValidation(leaderboardSchemas.record),
  async (req, res) => {
    const payload = req.body as InferBody<typeof leaderboardSchemas.record>;
